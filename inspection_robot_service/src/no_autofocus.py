...
class PoseStampedCreator(Node):
    ...
    def send_request_world(self,pose_map):
        ...
        target_pose_map = PoseStamped()
        target_pose_map.header.frame_id = 'world'
        target_pose_map.pose = pose_map.pose
        self.req.target_pose = target_pose_map
    ...
    def simple_feedback(self):
        # Test moving to a random, defined pose
        pose_tool0 = PoseStamped()
        pose_tool0.header.stamp = self.get_clock().now().to_msg()
        pose_tool0.header.frame_id = 'tool0'
        pose_tool0.pose.position.x = 0.6202010554762885
        pose_tool0.pose.position.y = -0.09394698702271967
        pose_tool0.pose.position.z = 0.22654127460344958
        pose_tool0.pose.orientation.w = 1.0
        self.send_request_world(pose_tool0)
        time.sleep(1)
        self.get_logger().info(f'Completed transform')
        time.sleep(1)
        return
